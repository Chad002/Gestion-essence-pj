...
if (isset($_POST['ajouterAchat'])) {
    ...
    $result = $conn->query("SELECT stock FROM PRODUIT WHERE numProd = '$numProd'");
    $row = $result->fetch_assoc();
    $stockDisponible = $row['stock'];

    // Vérifier si le stock est suffisant
    if ($nbrLitre > $stockDisponible) {
        $_SESSION['message'] = 'Erreur : Le stock disponible est insuffisant !';
        ...
        if ($conn->query($sql)) {
            ...
            $conn->query("UPDATE PRODUIT SET stock = stock - $nbrLitre WHERE numProd = '$numProd'");
...
if (isset($_POST['modifierAchat'])) {
    ...
    // Récupérer l'ancien nombre de litres
    $result = $conn->query("SELECT nbrLitre FROM ACHAT WHERE numAchat = '$numAchat'");
    $row = $result->fetch_assoc();
    $oldNbrLitre = $row['nbrLitre'];
    ...
    $litreDifference = $nbrLitre - $oldNbrLitre;

    // Vérifier si le stock est suffisant
    if ($litreDifference > $stockDisponible) {
        $_SESSION['message'] = 'Erreur : Le stock disponible est insuffisant pour cette modification !';
        $_SESSION['message_type'] = 'error';
    } else {
        // Mettre à jour l'achat dans la table ACHAT
        $sql = "UPDATE ACHAT SET numProd='$numProd', nomClient='$nomClient', nbrLitre=$nbrLitre, dateAchat='$dateAchat' WHERE numAchat='$numAchat'";
        
        if ($conn->query($sql)) {
            // Mettre à jour le stock dans la table PRODUIT
            $conn->query("UPDATE PRODUIT SET stock = stock - $litreDifference WHERE numProd = '$numProd'");
...
if (isset($_GET['supprimer'])) {
    ...
    // Récupérer le nombre de litres et le numéro de produit
    $result = $conn->query("SELECT nbrLitre, numProd FROM ACHAT WHERE numAchat = '$numAchat'");
    $row = $result->fetch_assoc();
    $nbrLitre = $row['nbrLitre'];
    $numProd = $row['numProd'];
...
<body>
    ...
    <div class="main-content">
        ...
        <h1>Gestion des Achats</h1><br><br>
